...
if (!empty($search)) {
    $query .= " AND kategori LIKE '%$search%'";
}

// Function to get user's transaction list
function getTransaksi($koneksi, $username, $search){
    $sql = "SELECT * FROM transaksi WHERE username='$username' AND kategori LIKE '%$search%'";
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<td class='border border-gray-300 px-4 py-2'>" . $row['keterangan'] . "</td>";
            echo "<td class='border border-gray-300 px-4 py-2'>" . $row['jumlah'] . "</td>";
            echo "<td class='border border-gray-300 px-4 py-2'>" . $row['tanggal'] . "</td>";
            echo "<td class='border border-gray-300 px-4 py-2'>" . $row['kategori'] . "</td>";
            echo "<td class='border border-gray-300 px-4 py-2'>";
            echo "<a type='button' class='bg-yellow-500 text-white px-4 py-2 rounded-md mr-2' href='edit.php?id=" . $row['id'] . "'>EDIT</a>";
            echo "<a type='button' class='bg-red-500 text-white px-4 py-2 rounded-md mr-2' href='hapus.php?id=" . $row['id'] . "' onclick='return confirmDelete()'>HAPUS</a>";
            echo "<a type='button' class='bg-blue-500 text-white px-4 py-2 rounded-md' href='detail.php?id=" . $row['id'] . "'>Detail transaksi</a>";
            echo "</td>";
            echo "</tr>";
        }
    } else {
...
<aside class="ml-[-100%] fixed z-10 top-0 pb-3 px-6 w-full flex flex-col justify-between h-screen border-r bg-white transition duration-300 md:w-4/12 lg:ml-0 lg:w-[25%] xl:w-[20%] 2xl:w-[15%]">
    <div>
        ...
        <ul class="space-y-2 tracking-wide mt-8">
            ...
            <li>
                <a href="transaksi.php" class="relative px-4 py-3 flex items-center space-x-4 rounded-xl text-white bg-gradient-to-r from-sky-600 to-cyan-400">
                 <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <a href="hutang.php" class="px-4 py-3 flex items-center space-x-4 rounded-md text-gray-600 group">
                    ...
                    <span class="group-hover:text-gray-700">Hutang</span>
                </a>
            </li>
            <li>
...
<div class="ml-auto mb-6 lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
    <div class="sticky z-10 top-0 h-16 border-b bg-blue-900 lg:py-2.5">
        <div class="px-6 flex items-center justify-between space-x-4 2xl:container">
            <h4 class="text-3xl text-gray-50 font-medium lg:block hidden lg:flex-grow text-center">Data Transaksi</h4>
            <h4 class="text-3xl text-gray-50 font-medium lg:hidden block text-center">Data Transaksi</h4>
     ...
     <div class="bg-white shadow-lg p-4 mb-8">
            <h2 class="text-lg font-semibold mb-4 text-center">Dihalaman ini Anda Bisa Mengelola Data Transaksi</h2>
        </div>
    <div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-4">
        <a type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" href="tambah.php">Tambah Transaksi</a>
    </div>
    <!--search bar -->
    <form action="" method="GET">
    <div class="flex items-center mb-4">
        <label for="search" class="mr-2">Cari berdasarkan kategori:</label>
        <select name="search" id="search" class="border border-gray-300 px-4 py-2 rounded-md">
            <option value="">Semua</option>
            <option value="Pendapatan" <?php if ($search === 'Pendapatan') echo 'selected'; ?>>Pendapatan</option>
            <option value="Pengeluaran" <?php if ($search === 'Pengeluaran') echo 'selected'; ?>>Pengeluaran</option>
        </select>
...
<table class="w-full bg-white border border-gray-300">
    <thead>
        <tr class="bg-gray-200">
            ...
            <th class="border border-gray-300 px-4 py-2">Nominal (Rp)</th>
    ...
    <tbody class="border border-gray-300 px-4 py-2 text-center">
        ...
        getTransaksi($koneksi, $username, $search);
        ?>
        </tbody>
</tbody>
</table>
